...
The site backend SHALL be the only application that calls Bonda. It SHALL attach
Bonda credentials, authenticate the Carobra customer, enforce canonical level
policy, and translate partner responses into stable Carobra contracts. The
frontend MUST use same-origin Carobra routes and MUST NOT receive a Bonda API
key, affiliate token, microsite identifier, or unrestricted partner URL.

#### Scenario: Browser loads the coupon catalog
- **WHEN** the authenticated frontend requests customer coupons
- **THEN** it calls a same-origin Carobra endpoint and the site backend performs any required Bonda read
...
allowlist and SHALL keep live partner reads disabled when required configuration
is missing. Affiliate provisioning and coupon-request controls MUST remain
disabled independently. Missing Bonda configuration MUST NOT prevent the
...
The customer shell SHALL expose Inicio, Beneficios, Cursos, Productos, and
Actividad as its primary destinations. It SHALL omit Ganar puntos, present
Cursos as a truthful Próximamente page, and expose Ayuda and Notificaciones as
accessible utility icons with visible hover and keyboard-focus labels.
